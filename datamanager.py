...
from utils import init_session, init_goal, state_vectorize, action_vectorize, usr_state_vectorize, usr_action_vectorize
...
class DataManager:
    ...
    def _create_dataset_usr(self, part, file_dir, cfg):
        datas = self.data[part]
        goals = self.goal[part]
        s, a, done = [], [], []
        for idx, turn_data in enumerate(datas):
            if turn_data['others']['turn'] % 2 == 1:  # sys action
                continue
            usr_goal = goals[turn_data['others']['session_id']]
            sys_action_list = list(turn_data['history']['sys'].keys()) + list(turn_data['sys_action'])
            s.append(torch.Tensor(usr_state_vectorize(usr_goal, sys_action_list, cfg)))
            a.append(torch.Tensor(usr_action_vectorize(turn_data['user_action'], cfg)))
            done.append(torch.Tensor([float(turn_data['others']['terminal'])]))
        torch.save((s, a, done), file_dir)
    ...
    def create_dataset_usr(self, part, batchsz, cfg):
        logging.debug('start loading user {}'.format(part))
        file_dir = self.data_dir_new + '/' + part + '_usr.pt'
        if not os.path.exists(file_dir):
            self._create_dataset_usr(part, file_dir, cfg)

        s, a, d = torch.load(file_dir)
        dataset = DatasetUser(s, a, d)
        dataloader = data.DataLoader(dataset, batchsz, True)
        logging.debug('finish loading user {}'.format(part))
        return dataloader
...
class DatasetUser(data.Dataset):
    def __init__(self, s_u, a_u, d_u):
        self.s_u = s_u
        self.a_u = a_u
        self.d_u = d_u
        self.num_total = len(s_u)
