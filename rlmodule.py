...
VanillaTransition = namedtuple('VanillaTransition', ('state', 'action', 'reward', 'mask', 'next_state'))
Transition = namedtuple('Transition', ('state', 'action', 'mask', 'next_state'))
UserAction = namedtuple('UserAction', ('state_comp', 'state_vec', 'action_vec', 'terminal'))
UserAction2 = namedtuple("UserAction2", ('state_comp', 'state_vec', 'action_vec', 'terminal', 'goal', 'agenda'))
...
class Memory(object):
    ...
    def get_batch(self, batch_size=None):
        if batch_size is None:
            return self.named_tuple(*zip(*self.memory))
        else:
            random_batch = random.sample(self.memory, batch_size)
            return self.named_tuple(*zip(*random_batch))
